...
const generate90Days = () => {
    ...
    addDay(7, "Pesagem de Ajuste Clínico", "[GAMIFICAÇÃO] ⚖️ Domingo de check-in oficial.\nQual o seu peso de jejum hoje (em kg)? Nós não queremos perda brusca, queremos constância. Registre aqui! 👇", true, null);

    addDay(8, "Avaliando a Engrenagem", "Segundou na Evolução! ☀️ Diferente do começo, agora a gente analisa os erros do final de semana em vez de apenas ignorá-los. O que você exagerou ontem que podemos fazer diferente no próximo domingo?");
    addDay(9, "Check-in: O Prato Perfeito", "[GAMIFICAÇÃO] 🍽️ Check-in Nutricional!\nComo estava o volume de salada e proteína no almoço?\nA) Metade do prato de salada, porção excelente de proteína!\nB) Tinha mais carboidrato que o ideal, mas segurei.\nC) Almocei muito mal hoje, comi rápido e sem pensar.", true, "alimentacao");
    ...
    for (let day = 22; day <= 90; day++) {
        ...
        if (dayOfWeek === 1) { // Monday
            ...
            addDay(day, `Check-in Visual e de Peso (Semana ${currentWeek})`, `[GAMIFICAÇÃO] ⚖️ Chegando ao fim da semana ${currentWeek}. Insira o seu peso de jejum hoje para o nosso sistema mapear seu platô ou descida!`, true, null);
        }
    }
    ...
    days[89].message = "[GAMIFICAÇÃO] ⚖️ Dia Histórico! O nível Diamante e a consagração dos seus 180 dias totais no aplicativo estão acontecendo. Digite seu peso final oficial para documentarmos 🏆!";
